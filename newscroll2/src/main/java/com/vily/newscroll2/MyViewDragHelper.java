...
/**
 *  * description :  重写 ViewDragHelper  ，为了充值duration
 *  
 **/
...
//    private static final int MAX_SETTLE_DURATION = 600;
    ...
    private final Runnable mSetIdleRunnable = new Runnable() {
        public void run() {
            MyViewDragHelper.this.setDragState(0);
        }
    };
    ...
    public static MyViewDragHelper create(@NonNull ViewGroup forParent, float sensitivity, @NonNull MyViewDragHelper.Callback cb) {
        MyViewDragHelper helper = create(forParent, cb);
        helper.mTouchSlop = (int)((float)helper.mTouchSlop * (1.0F / sensitivity));
        return helper;
    }

    private MyViewDragHelper(@NonNull Context context, @NonNull ViewGroup forParent, @NonNull MyViewDragHelper.Callback cb) {
        if (forParent == null) {
    ...
    private boolean forceSettleCapturedViewAt(int finalLeft, int finalTop, int xvel, int yvel) {
        ...
        if (dx == 0 && dy == 0) {
            ...
            this.mScroller.startScroll(startLeft, startTop, dx, dy, ANI_DURATION);
            this.setDragState(2);
            return true;
        }
    }
